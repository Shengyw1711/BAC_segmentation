...
img_gray=img_origin;%灰度化
Sigma=1.5;%σ?高斯模糊半?，半?越大模糊程度越大
...
WeightMatrix=WeightMatrix./sum(sum(WeightMatrix)) %使?3*3矩?之和等于1
[row, col] = size( img_gray );

for i = 1: row  % 垂直方向
    for j = 1:col  % 水平方向 
        if i==1 || j==1 || i==row || j==col
            img_undist(i, j)=img_gray(i, j);%??未?理
        else
